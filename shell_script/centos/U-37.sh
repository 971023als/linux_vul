#!/bin/bash
# shell_script/centos/U-37.sh
# -----------------------------------------------------------------------------
# [U-37] 웹 서비스 상위 디렉터리 접근 제한 (CentOS/RHEL/Oracle)
# -----------------------------------------------------------------------------
...
if [ -f "$HTTPD_CONF" ]; then
    # AllowOverride None 및 심볼릭 링크 허용(FollowSymLinks) 여부 확인
    ...
    if grep -r "Options" /etc/httpd/ 2>/dev/null | grep -qi "FollowSymLinks"; then
        # 취약할 수 있으나 실무상 사용되는 경우 많음. 보안 가이드는 비활성 권고
