#!/bin/bash
# shell_script/dbms/mysql/DBM-005.sh
# [DBM-005] 로그인 실패 횟수에 따른 접속 제한 설정 – mysql
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/mysql/failed_login_policy.txt password_policy.txt 필요"
    exit 0
fi
