#!/bin/bash
# shell_script/dbms/mysql/DBM-013.sh
# [DBM-013] Public Role에 불필요한 권한 제거 – mysql
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/mysql/public_role_privileges.txt roles.txt 필요"
    exit 0
fi
