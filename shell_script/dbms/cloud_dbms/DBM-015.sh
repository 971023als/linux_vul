#!/bin/bash
# shell_script/dbms/cloud_dbms/DBM-015.sh
# [DBM-015] 업무상 불필요한 시스템 테이블 접근 권한 제거 – cloud_dbms
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/cloud_dbms/system_table_privileges.txt 필요"
    exit 0
fi
