#!/bin/bash
# shell_script/dbms/cloud_dbms/DBM-013.sh
# [DBM-013] Public Role에 불필요한 권한 제거 – cloud_dbms
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/cloud_dbms/public_role_privileges.txt roles.txt 필요"
    exit 0
fi
