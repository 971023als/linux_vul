#!/bin/bash
# shell_script/dbms/altibase/DBM-020.sh
# [DBM-020] 불필요하게 WITH GRANT OPTION 옵션이 설정된 권한 제거 – altibase
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/altibase/roles.txt public_role_privileges.txt 필요"
    exit 0
fi
