#!/bin/bash
# shell_script/dbms/altibase/DBM-019.sh
# [DBM-019] 설정 파일 및 중요정보가 포함된 파일의 접근 권한 설정 적절성 – altibase
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/altibase/file_permissions.txt 필요"
    exit 0
fi
