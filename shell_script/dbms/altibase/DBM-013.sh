#!/bin/bash
# shell_script/dbms/altibase/DBM-013.sh
# [DBM-013] Public Role에 불필요한 권한 제거 – altibase
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/altibase/public_role_privileges.txt roles.txt 필요"
    exit 0
fi
