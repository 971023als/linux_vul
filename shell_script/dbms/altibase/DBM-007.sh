#!/bin/bash
# shell_script/dbms/altibase/DBM-007.sh
# [DBM-007] 비밀번호 변경 주기 충족 여부 – altibase
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/altibase/password_policy.txt password_lifetime.txt 필요"
    exit 0
fi
