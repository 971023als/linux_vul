#!/bin/bash
# shell_script/dbms/mssql/DBM-013.sh
# [DBM-013] Public Role에 불필요한 권한 제거 – mssql
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/mssql/public_role_privileges.txt roles.txt 필요"
    exit 0
fi
