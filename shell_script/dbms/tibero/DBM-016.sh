#!/bin/bash
# shell_script/dbms/tibero/DBM-016.sh
# [DBM-016] 이전 비밀번호 재사용 요구사항 충족 여부 – tibero
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/tibero/password_policy.txt password_reuse_policy.txt 필요"
    exit 0
fi
