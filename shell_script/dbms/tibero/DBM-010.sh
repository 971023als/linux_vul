#!/bin/bash
# shell_script/dbms/tibero/DBM-010.sh
# [DBM-010] Listener Control Utility(lsnrctl) 보안 설정 여부 – tibero
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/tibero/listener_config.txt 필요"
    exit 0
fi
