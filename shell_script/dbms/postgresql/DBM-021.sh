#!/bin/bash
# shell_script/dbms/postgresql/DBM-021.sh
# [DBM-021] 서비스 지원이 종료된 EOS 시스템 및 장비 교체 여부 – postgresql
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/postgresql/patch_status.txt 필요"
    exit 0
fi
