#!/bin/bash
# shell_script/dbms/postgresql/DBM-012.sh
# [DBM-012] 취약한 운영체제 역할 인증 기능 비활성화 – postgresql
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/postgresql/remote_access.txt roles.txt 필요"
    exit 0
fi
