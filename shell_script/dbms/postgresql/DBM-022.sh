#!/bin/bash
# shell_script/dbms/postgresql/DBM-022.sh
# [DBM-022] 데이터베이스 구동 계정의 umask 설정 미흡 – postgresql
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/postgresql/service_account.txt 필요"
    exit 0
fi
