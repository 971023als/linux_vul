#!/bin/bash
# shell_script/dbms/postgresql/DBM-027.sh
# [DBM-027] 데이터베이스 접속 시 통신구간에 비밀번호 평문 노출 – postgresql
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "EVIDENCE=input/evidence/dbms/postgresql/network_encryption.txt 필요"
    exit 0
fi
