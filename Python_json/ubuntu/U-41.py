...
def main():
    ...
    for server_name, server_info in web_servers.items():
        ...
        document_root_set, vulnerable = check_document_root_settings(server_info, found_files)
        overall_document_root_set |= document_root_set
        overall_vulnerable |= vulnerable

        if vulnerable:
            results["현황"].append(f"{server_name}의 DocumentRoot가 기본 디렉터리로 설정되어 있습니다.")

    if not overall_document_root_set:
        results["진단 결과"] = "취약"
        results["현황"].append("웹서비스의 DocumentRoot가 설정되지 않았습니다.")
    elif overall_vulnerable:
        results["진단 결과"] = "취약"
    else:
        results["진단 결과"] = "양호"
