    ...
    for file_path in found_files:
        if file_path:
            with open(file_path, 'r') as file:
                content = file.read()
                if (server_info['link_restriction_directive'] in content and
                    server_info['correct_restriction_setting'] not in content):
...
def main():
    ...
    for server_name, server_info in web_servers.items():
        print(f"\nChecking {server_name} for symbolic link usage restrictions...")
